...
export default function HistoryPanel({ isOpen, onClose }) {
    ...
    return (
        <>
            ...
            {/* パネル */}
            <div className="fixed inset-y-0 right-0 w-full max-w-2xl glass border-l border-slate-700/50 z-50 flex flex-col animate-slide-in-right">
                {/* ヘッダー */}
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <button
                                ...
                                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg hover:bg-rose-500/20 text-rose-400 text-sm transition-colors"
                                title="すべて削除"
                            >
                                ...
                                <>
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                        <div className="p-4 rounded-xl bg-slate-800/50 border border-slate-700/50">
                                            ...
                                            <div className="text-xs text-slate-400">総検索回数</div>
                                        </div>
                                        <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30">
                                            <div className="text-2xl font-bold text-emerald-400">{stats.successfulSearches}</div>
                                            <div className="text-xs text-slate-400">成功</div>
                                        </div>
                                        <div className="p-4 rounded-xl bg-rose-500/10 border border-rose-500/30">
                                            <div className="text-2xl font-bold text-rose-400">{stats.failedSearches}</div>
                                            <div className="text-xs text-slate-400">失敗</div>
                                        </div>
                                        <div className="p-4 rounded-xl bg-indigo-500/10 border border-indigo-500/30">
                                            <div className="text-2xl font-bold text-indigo-400">{stats.totalApisFound}</div>
                                            <div className="text-xs text-slate-400">発見したAPI</div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-4 rounded-xl bg-slate-800/50 border border-slate-700/50">
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            <div>
                                                <div className="text-lg font-bold text-white">{formatTime(stats.averageProcessingTime)}</div>
                                                <div className="text-xs text-slate-400">平均処理時間</div>
                                            </div>
                                            <div>
                                                <div className="text-lg font-bold text-white">{stats.totalTokensUsed.toLocaleString()}</div>
                                                <div className="text-xs text-slate-400">総トークン使用量</div>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div className="p-4 rounded-xl bg-slate-800/50 border border-slate-700/50">
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {stats.topKeywords.map(({ keyword, count }) => (
                                                    <span
                                                        key={keyword}
                                                        className="px-3 py-1.5 rounded-full bg-indigo-500/20 text-indigo-300 text-sm"
                                                    >
                                                        {keyword}
                                                        <span className="ml-1.5 text-indigo-400/70">({count})</span>
                                                    </span>
                                                ))}
                                            </div>
                                        </div>
                                                    ...
                                                    <div className="px-4 pb-4 border-t border-slate-700/50 pt-4 space-y-4">
                                                        ...
                                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                                                            <div className="p-2 rounded-lg bg-slate-900/50">
                                                                <div className="text-xs text-slate-500 mb-1">モデル</div>
                                                        ...
                                                        {entry.error && (
                                                            <div className="p-3 rounded-lg bg-rose-500/10 border border-rose-500/30">
                                                                <div className="text-xs text-rose-400 font-medium mb-1">エラー</div>
                                                                <div className="text-sm text-rose-300">{entry.error}</div>
                                                            </div>
                                                        )}
                                                        ...
                                                        <div>
                                                            ...
                                                            <pre className="p-3 rounded-lg bg-slate-900/80 text-xs text-slate-300 overflow-x-auto max-h-40 overflow-y-auto">
                                                                {entry.prompt}
                                                            </pre>
                                                        </div>
                                                        ...
                                                        {entry.response && (
                                                            <div>
                                                                ...
                                                                <pre className="p-3 rounded-lg bg-slate-900/80 text-xs text-slate-300 overflow-x-auto max-h-60 overflow-y-auto">
                                                                    {entry.response}
                                                                </pre>
                                                            </div>
                                                        )}
